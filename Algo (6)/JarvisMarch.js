...
function OrientationCheck(p, q, r) {
...
function compare(a, b) { 
    var p = bottomMost;
    var o = OrientationCheck(p, a, b); 
    if (o == 0) 
        return (CalculateDistance(p, b) >= CalculateDistance(p, a))? -1 : 1; 
   return (o == -1)? -1: 1; 
}
...
function drawLines(txt, stack, condition) {
    ...
    if (!condition) {
        for (var i = 0; i < stack.length - 1; i++) {    
            x = x + "<svg width='1800' height='1000' style='position: absolute'><line x1='" + (stack[i][0] + 2)+ "' y1='" + (stack[i][1] + 2) + "' x2='" + (stack[i + 1][0] + 2) + "' y2='" + (stack[i + 1][1] + 2) + "' stroke='blue'/></svg>";
        }
    } else {
        for (var i = 0; i < stack.length - 1; i++) {  
...
async function findConvexHull(txt, points) {
    console.log("check");
    console.log(points);
    ...
    while(true) {
        ...
        if (OrientationCheck(stack[count - 2], stack[count - 1], points[tracker]) == -1) {
...
function generateNumbers(inside) {
...
function PlottingPoints(total) {
    points = [];
    text = "";
    for (let i = 0; i < total; i++) {
        points.push(
            [
                generateNumbers("left"),
                generateNumbers("top")
            ]
        );
    }
...
function JarvisMarch(n) {
    ...
    var stack = findConvexHull(text, points);
    console.log(stack);
}