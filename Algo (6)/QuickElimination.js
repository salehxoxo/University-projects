...
function draw(text, s, t, dict) {
    for(var i = 0; i < s.length; i++) {
        var r = Math.floor(s[i][0] + s[i + 1][0]);
        dict[r] = generateString();
        text = text + "<div id='" + dict[r] + "'><svg  width='1800' height='1000' style='position: absolute'><line x1='" + (s[i][0] + 2)+ "' y1='" + (s[i][1] + 2) + "' x2='" + (s[i + 1][0] + 2) + "' y2='" + (s[i + 1][1] + 2) + "' stroke='black'/></svg></div>";
        i = i + 1;
    }
    document.getElementById("canvas").innerHTML = text;
    for (var i = 0; i < t.length; i = i + 2) {
